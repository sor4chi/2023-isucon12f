...
func (h *Handler) adminSessionCheckMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.DB1.Get(adminSession, query, sessID); err != nil {
...
func (h *Handler) adminListMaster(c echo.Context) error {
	...
	if err := h.DB1.Select(&masterVersions, "SELECT * FROM version_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	items := make([]*ItemMaster, 0)
	if err := h.DB1.Select(&items, "SELECT * FROM item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachas := make([]*GachaMaster, 0)
	if err := h.DB1.Select(&gachas, "SELECT * FROM gacha_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachaItems := make([]*GachaItemMaster, 0)
	if err := h.DB1.Select(&gachaItems, "SELECT * FROM gacha_item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	presentAlls := make([]*PresentAllMaster, 0)
	if err := h.DB1.Select(&presentAlls, "SELECT * FROM present_all_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonuses := make([]*LoginBonusMaster, 0)
	if err := h.DB1.Select(&loginBonuses, "SELECT * FROM login_bonus_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonusRewards := make([]*LoginBonusRewardMaster, 0)
	if err := h.DB1.Select(&loginBonusRewards, "SELECT * FROM login_bonus_reward_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) adminUser(c echo.Context) error {
	...
	if err = h.getDBByUserId(userID).Select(&devices, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_cards WHERE user_id=?"
	cards := make([]*UserCard, 0)
	if err = h.getDBByUserId(userID).Select(&cards, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_decks WHERE user_id=?"
	decks := make([]*UserDeck, 0)
	if err = h.getDBByUserId(userID).Select(&decks, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_items WHERE user_id=?"
	items := make([]*UserItem, 0)
	if err = h.getDBByUserId(userID).Select(&items, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_login_bonuses WHERE user_id=?"
	loginBonuses := make([]*UserLoginBonus, 0)
	if err = h.getDBByUserId(userID).Select(&loginBonuses, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_presents WHERE user_id=?"
	presents := make([]*UserPresent, 0)
	if err = h.getDBByUserId(userID).Select(&presents, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	query = "SELECT * FROM user_present_all_received_history WHERE user_id=?"
	presentHistory := make([]*UserPresentAllReceivedHistory, 0)
	if err = h.getDBByUserId(userID).Select(&presentHistory, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) adminBanUser(c echo.Context) error {
	...
	if _, err = h.getDBByUserId(userID).Exec(query, banID, userID, requestAt, requestAt, requestAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
