...
func main() {
	...
	dbx1, err := connectDB(false, DB1)
	if err != nil {
		e.Logger.Fatalf("failed to connect to db: %v", err)
	}
	defer dbx1.Close()
	dbx2, err := connectDB(false, DB2)
	if err != nil {
		e.Logger.Fatalf("failed to connect to db: %v", err)
	}
	defer dbx2.Close()
	dbx3, err := connectDB(false, DB3)
	if err != nil {
		e.Logger.Fatalf("failed to connect to db: %v", err)
	}
	defer dbx3.Close()
	dbx4, err := connectDB(false, DB4)
	if err != nil {
		e.Logger.Fatalf("failed to connect to db: %v", err)
	}
	defer dbx4.Close()
...
// connectDB DBに接続する
func connectDB(batch bool, db DB) (*sqlx.DB, error) {
	dsn := fmt.Sprintf(
...
func (h *Handler) apiMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.DB1.Get(masterVersion, query); err != nil {
...
func (h *Handler) checkSessionMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.getDBByUserId(userID).Get(userSession, query, sessID); err != nil {
			if err == sql.ErrNoRows {
				return errorResponse(c, http.StatusUnauthorized, ErrUnauthorized)
			}
			return errorResponse(c, http.StatusInternalServerError, err)
		...
		if userSession.ExpiredAt < requestAt {
			...
			if _, err = h.getDBByUserId(userID).Exec(query, requestAt, sessID); err != nil {
				return errorResponse(c, http.StatusInternalServerError, err)
			}
...
func (h *Handler) checkOneTimeToken(token string, tokenType int, requestAt int64) error {
	...
	if err := h.DB1.Get(tk, query, token, tokenType); err != nil {
		if err == sql.ErrNoRows {
	...
	if tk.ExpiredAt < requestAt {
		...
		if _, err := h.DB1.Exec(query, requestAt, token); err != nil {
			return err
		}
		return ErrInvalidToken
	}
...
func (h *Handler) checkViewerID(userID int64, viewerID string) error {
	...
	if err := h.getDBByUserId(userID).Get(device, query, userID, viewerID); err != nil {
		if err == sql.ErrNoRows {
...
func (h *Handler) checkBan(userID int64) (bool, error) {
	...
	if err := h.getDBByUserId(userID).Get(banUser, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return false, nil
		}
		return false, err
...
func initialize(c echo.Context) error {
	DBs := []DB{DB1, DB2, DB3, DB4}
	wg := sync.WaitGroup{}
	for _, db := range DBs {
		wg.Add(1)
		go func(db DB) {
			defer wg.Done()
			dbx, err := connectDB(true, db)
			if err != nil {
			...
			go func() {
				defer wg.Done()
				_, err = dbx.Exec("ALTER TABLE user_present_all_received_history ADD INDEX idx_user_present_all_received_history_user_id (user_id, present_all_id)")
				if err != nil {
					log.Printf("[INFO] Already added index idx_user_present_all_received_history_user_id", err)
				}
			}()
...
func (h *Handler) login(c echo.Context) error {
	...
	user := new(User)
	query := "SELECT * FROM users WHERE id=?"
	if err := h.DB1.Get(user, query, req.UserID); err != nil {
		if err == sql.ErrNoRows {
	...
	tx, err := h.getDBByUserId(user.ID).Beginx()
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	defer tx.Rollback() //nolint:errcheck
...
func (h *Handler) listGacha(c echo.Context) error {
	...
	err = h.DB1.Select(&gachaMasterList, query, requestAt, requestAt)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	...
	for _, v := range gachaMasterList {
		...
		err = h.DB1.Select(&gachaItem, query, v.ID)
		if err != nil {
			return errorResponse(c, http.StatusInternalServerError, err)
		}
	...
	if _, err = h.getDBByUserId(userID).Exec(query, requestAt, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	...
	if _, err = h.getDBByUserId(userID).Exec(query, token.ID, token.UserID, token.Token, token.TokenType, token.CreatedAt, token.UpdatedAt, token.ExpiredAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) drawGacha(c echo.Context) error {
	...
	user := new(User)
	query := "SELECT * FROM users WHERE id=?"
	if err := h.DB1.Get(user, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, ErrUserNotFound)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	if err = h.DB1.Get(gachaInfo, query, gachaID, requestAt, requestAt); err != nil {
	...
	err = h.DB1.Select(&gachaItemList, "SELECT * FROM gacha_item_masters WHERE gacha_id=? ORDER BY id ASC", gachaID)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	...
	var sum int64
	err = h.DB1.Get(&sum, "SELECT SUM(weight) FROM gacha_item_masters WHERE gacha_id=?", gachaID)
	if err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
	...
	tx, err := h.getDBByUserId(userID).Beginx()
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	defer tx.Rollback() //nolint:errcheck
...
func (h *Handler) listItem(c echo.Context) error {
	...
	if err = h.getDBByUserId(userID).Select(&itemList, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	cardList := make([]*UserCard, 0)
	query = "SELECT * FROM user_cards WHERE user_id=?"
	if err = h.getDBByUserId(userID).Select(&cardList, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) addExpToCard(c echo.Context) error {
	...
	`
	if err = h.getDBByUserId(userID).Get(card, query, cardID, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	for _, v := range req.Items {
		...
		if err = h.getDBByUserId(userID).Get(item, query, v.ID, userID); err != nil {
			if err == sql.ErrNoRows {
				return errorResponse(c, http.StatusNotFound, err)
			}
			return errorResponse(c, http.StatusInternalServerError, err)
...
func (h *Handler) updateDeck(c echo.Context) error {
	...
	if err = h.getDBByUserId(userID).Select(&cards, query, params...); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) reward(c echo.Context) error {
	...
	user := new(User)
	query := "SELECT * FROM users WHERE id=?"
	if err = h.getDBByUserId(userID).Get(user, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, ErrUserNotFound)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	if err = h.getDBByUserId(userID).Get(deck, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	cards := make([]*UserCard, 0)
	query = "SELECT * FROM user_cards WHERE id IN (?, ?, ?)"
	if err = h.getDBByUserId(userID).Select(&cards, query, deck.CardID1, deck.CardID2, deck.CardID3); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	if len(cards) != 3 {
	...
	if _, err = h.getDBByUserId(userID).Exec(query, user.IsuCoin, user.LastGetRewardAt, user.ID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
